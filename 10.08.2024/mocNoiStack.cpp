...
// Định nghĩa kiểu Snode là con trỏ đến struct Node
...
void push(Snode* top, int data) {
    ...
    newNode->next = *top;  // Nối node mới vào đầu stack
...
int pop(Snode* top) {
    ...
    *top = (*top)->next;  // Cập nhật top
    free(temp);           // Giải phóng bộ nhớ cho node bị xóa
    return poppedData;
}
...
int main() {
    Snode stack = createNode(50);  // Khởi tạo stack rỗng
    display(stack);
    push(&stack, 10);
    display(stack);
    push(&stack, 20);
